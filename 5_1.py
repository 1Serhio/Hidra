...
#l - длина области по x; t_s - длина расчетного времени; n - кол-во узлов по x; m - кол-во временных слоёв; u0 - н.у.; mu - гр. условия на левой краю
def create_grid(length, time_slice, n, m, u0, mu):
    grid = []

    h = length / n #шаг по x
    t = time_slice / m #шаг по времени
    #создание пустой (0) сетки
    for j in range(m):
        grid.append([0] * n)
    #заполнение н.у. при t = 0
    for i in range(n):
        grid[0][i] = u0(i * h)
    #заполнение гр. условий на левой краю
    for j in range(m):
        grid[j][0] = mu(j * t)

    return grid, h, t

#н.у. - прямоугольник; на отрезке 1 <= x <= 2 u(x,0) = 1, иначе 0
def u0_0(x):
...
#гр. условия на левой краю
def mu(t):
    return 0

def solve_single(t, grid, nu, h, tau):
    """Неявная схема для уравнения Бюргерса"""
    prev = grid[t - 1] #слой на момент t-1
    curr = grid[t] #слой на момент t
    n = len(curr)

    # Коэффициенты схемы
    c1 = 1.0 / tau #для производной по времени
    c2 = 1.0 / (2 * h)  # для конвективного члена (производная по x)
    c3 = nu / (h ** 2)  # для диффузионного члена (вторая производная по x)
